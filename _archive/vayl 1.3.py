...
USER_SCOPE = [AuthScope.USER_READ_SUBSCRIPTIONS, AuthScope.MODERATION_READ, AuthScope.CHANNEL_READ_REDEMPTIONS, 
              ...
              AuthScope.CHANNEL_READ_HYPE_TRAIN, AuthScope.CHANNEL_MANAGE_VIPS, AuthScope.CHANNEL_MANAGE_BROADCAST]
TARGET_CHANNEL = ''
...
async def on_ready(ready_event: EventData):
    ...
    # await ready_event.chat.send_message(TARGET_CHANNEL, "Testing")
    await reload(False)
...
async def custom_command (cmd: ChatCommand):
    ...
    try:
        ...
        with open(os.getcwd() + "\\configuration\\commands.yml", 'r', encoding = "utf-8") as file:
            ...
            if time.time() - commands[command]["user-cooldown"][user] >= data["command"][command]["cooldown"]:
...
async def runActions (actions, variables):
    ...
    for action in actions:
        ...
        if "playsound" in arguments[0]:
            try:
                threading.Thread(target=playsound, args=(os.getcwd() + "\\resources\\sounds\\" + arguments[1],), daemon=True).start()
            except:
                prompt ("error", "Error occured performing 'playsound' action.")
                await logError()
        ...
        if "tts" in arguments[0]:
            ...
            try:
                ...
                if data:
                    ...
                    with open(os.getcwd() + "\\" + filename, "+wb", encoding = "utf-8") as ttsfile:
                        ttsfile.write(data)
                    ...
                    if seconds <= int(cutoff):    
                        threading.Thread(target=playsound, args=(os.getcwd() + "\\" + filename,), daemon=True).start()
                        if halt.lower() == "true":
                            await asyncio.sleep(seconds)
                    else:
                        prompt("misc", "TTS ignored, over cutoff limit of " + cutoff + " seconds. (" + str(seconds) + "s)")
            ...
            if len(arguments) >= 3:
                if "sub" == arguments[0]:
                    if arguments[1].isalnum():
                        ...
                        alert["tier"] = "Tier 1"
                        alert["user"] = arguments[1]
                        alert["total-months"] = arguments[2]
                ...
                if "giftsub" == arguments[0]:
                    if arguments[1].isalnum():
                        if arguments[2].isnumeric():
                            ...
                            for i in range(0, int(arguments[2])):
                                alert = {}
                                alert["type"] = "giftsub"
...
async def callback_sub(d, data):
    ...
    try:
        ...
        sub_tier = {"Prime":"Prime","1000":"Tier 1","2000":"Tier 2","3000":"Tier 3"}
...
def triggerhotkey(name):
    prompt ("hotkey", "HotKey Triggered: " + name)
    with open(os.getcwd() + "\\configuration\\hotkeys.yml", 'r', encoding = "utf-8") as file:
        data = yaml.full_load(file)
        actions = data["hotkey"][name]["actions"]
        asyncio.run(runActions(actions, []))
    ...
    try:
        ...
        with open(os.getcwd() + "\\configuration\\hotkeys.yml", 'r', encoding = "utf-8") as file:
            ...
            if "enabled" in data and data["enabled"]:
                ...
                for hotkey in data["hotkey"].keys():
                    bindings.append([data["hotkey"][hotkey]["keys"], None, triggerhotkey, False, hotkey])

                register_hotkeys(bindings)
                start_checking_hotkeys()

                prompt ("success", "Custom HotKeys loaded. (" + str(len(data["hotkey"].keys())) + ")")
            else:
                prompt ("misc", "NO HotKeys loaded. (disabled in config)")
                
    except Exception as e:
        prompt ("error", "Error loading Custom Hotkeys.")
        asyncio.run(logError())
