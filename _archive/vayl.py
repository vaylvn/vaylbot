            ...
            try:
                with open(os.getcwd() + "\\configuration\\conditional-actions.yml", 'r', encoding = "utf-8") as file:
                    ...
                    if adata["name"] in data["conditionals"]:
                        ...
                        if "uptime;full" in condition.lower():
                            uptime = streams.started_at.replace(tzinfo=pytz.UTC)
                            now = datetime.now(tz=pytz.UTC)
                            days, remainder = divmod((now - uptime).total_seconds(), 86400)
                            hours, remainder = divmod(remainder, 3600)
                            ...
                            if "contains" in condition.lower():
                                c = {"seconds" : value in str(int(remainder)), "minutes" : value in str(int(minutes)), "hours" : value in str(int(hours)), "days" : value in str(int(days))}
                                for k,v in c.items():
                                    if k in condition.lower():
                                        result = v
                            
                                    
                            result = (value in ":".join([str(int(days)),str(int(hours)),str(int(minutes)),str(int(remainder))]))
                            
                            
                            
                            if "equals" in condition.lower():
                                result = value == ":".join([str(int(days)),str(int(hours)),str(int(minutes)),str(int(remainder))])
                        
                        if "uptime;seconds" in condition.lower():
                            uptime = streams.started_at.replace(tzinfo=pytz.UTC)
                            now = datetime.now(tz=pytz.UTC)
                            seconds = int((now - uptime).total_seconds())
                            
                            c = { "equals" : (value == seconds), "less than" : (value < seconds), "less than or equal" : (value <= seconds), "more than or equal" : (value >= seconds), "more than" : (value > seconds) }
                            for k,v in c.items():
                                if k in condition.lower():
                                    result = v
                        ...
                        if "time" in condition.lower():
                            ...
                            c = {"time is before":(current_time < value), "time is or before":(current_time <= value), "time equals":(current_time == value), "time is after":(current_time > value), "time is or after":(current_time >= value)}
                            for k,v in c.items():
                                if k in condition.lower():
                                    result = v
                        ...
                        if "counter" in condition.lower():
                            ...
                            c = {"less than value":(variable < value), "less than or equals value":(variable <= value), "equals value":(variable == value), "more than or equals value":(variable >= value), "more than value":(variable > value)}
                            for k,v in c.items():
                                if k in condition.lower():
                                    result = v
                        ...
                        if "text" in condition.lower():
                            value = str(value)
                            c = {"equals":(variable == value), "contains":(value in variable)}
                            for k,v in c.items():
                                if k in condition.lower():
                                    result = v
                        ...
                        if "list" in condition.lower():
                            value = str(value)
                            c = {"contains":(value in variable.splitlines())}
                            for k,v in c.items():
                                if k in condition.lower():
                                    result = v
