...
async def versionCheck():
    ...
    for v in soup.findAll("div", attrs={"class": "KL4NAf"}):
        ...
        '''
        # csplit = vayl_version.split(".")
        lsplit = latest.split(".")
        print (lsplit)
        
        
        if len(latest_split) > 0:
            if int(lsplit[0]) > int(latest_split[0]):
                latest_split = lsplit
                print ("set version to " + latest_split)
            elif int(lsplit[0]) == int(latest_split[0]):
                if int(lsplit[1]) > int(latest_split[1]):
                    latest_split = lsplit
                    print ("set version to " + latest_split)
                elif int(lsplit[1]) == int(latest_split[1]):
                    print ("bleep")
                    if int(lsplit[2]) > int(latest_split[2]):
                        latest_split = lsplit
                        print ("set version to " + latest_split)
...
async def on_message(msg: ChatMessage):
    ...
    try:
        ...
        ## First Chat        
        fm = msg.__dict__["_parsed"]["tags"]["first-msg"]
        first_message = True if fm == "1" else False
        
        if first_message:
            with open(os.getcwd() + "\\configuration\\event\\first-time-chat.yml", 'r', encoding = "utf-8") as file:
                data = yaml.safe_load(file)
                ...
                if data["enabled"]:
                    if data["queue"]:
                        ...
                        alert = {}
                        alert["type"] = "firstchat"
                        alert["user"] = msg.user.name
                        alert["buffer"] = data["buffer"]
                            ...
                            if data["enabled"]:
                                if name in data["user"]:
                                    ...
                                    if data["queue"]:
                                        ...
                                        alert["buffer"] = 0
                                        alert["actions"] = data["user"][name]["actions"]
                                        alert_queue.append(alert)
                                        ...
                                        variables = {}
                                        variables["user"] = msg.user.name
                                        actions = data["user"][name]["actions"]
                                        await runActions(actions, variables)
    ...
    for action in actions:
        ...
        if "timeout" in arguments [0]:
            try:
                ...
                time = int(arguments[2])
                
 
                
                async for u in twitch.get_users(logins = [user]):
                
                    await twitch.ban_user(streamer.id, streamer.id, u.id, "Vayl", time)
                    ...
                    if arguments[1].isalnum():
                        if arguments[2].isnumeric():
                            if arguments[3].isnumeric():
                                ...
                                tiers = ["1","2","3","Prime"]
                                for tier in tiers:
                                    if arguments[2].lower() == tier.lower():
                                        alert["tier"] = tier
                                        tier_found = True
                                if not tier_found:
                                    alert["tier"] = "null"
