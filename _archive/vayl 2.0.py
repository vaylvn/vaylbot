...
sv = { "id" : "xfc4596ekgo4ewkag6wn01hgs4hfbl", "secret" : "p8wl2zzuk3sgjmbdrlxe9l65xno8wk",
       "version" : "1.4", "twitch" : None, "streamer" : None, "channel" : None, "chat" : None, "live" : False,
...
async def on_message (msg: ChatMessage):
    ...
    if name.lower() != "vaylbot":
        ...
        if len(flagged) > 0:
            if not await isModerator(name) and not await isStreamer(name) and name.lower() not in sv["moderation"]["link"]["permitted-users"]:
        ...
        if percentage >= sv["moderation"]["cap"]["threshold-percentage"]:
            if not await isModerator(name) and not await is_streamer(name) and name.lower() not in sv["moderation"]["link"]["permitted-users"]:
    ...
    if not sv["live"]:
        ...
        if name not in sv["spoken"]:
            ...
            await addAlert({"type":"first-session-chat", "user":name},"0")
        ## =============================================================================
        
        ## first time chat =============================================================
        if msg.__dict__["_parsed"]["tags"]["first-msg"] == "1":
            await addAlert({"type":"first-time-chat", "user":name},"0")
        ## =============================================================================
...
async def on_poll_start (data: ChannelPollBeginEvent):
    ...
    id = 1
    for choice in event.__dict__["choices"]:
...
async def on_redeem (d, data):
    ...
    alert = {"type":"redeem", "userid":redeem["user"]["id"], "user":redeem["user"]["display_name"], "userinput":""}
    ...
    with open(os.getcwd() + "\\configuration\\redeems.yml", 'r', encoding = "utf-8") as file:
        ...
        for redeems in redeem_data["redeem"].keys():
            if redeem.lower() in redeem["reward"]["title"].lower():
...
async def c_debug (cmd: ChatCommand):
    ...
    if await isStreamer(cmd.user.name):
        if len(args) == 1:
            ...
            elif "sub" in args[0] and args[1].isalnum() and args[2].isnumeric() and args[3].isnumeric():
                tier = args[2].lower if args[2].lower() in ["1","2","3","prime"] else "1"
                await addAlert({"type":"sub","tier":tier,"user":args[1],"total-months":args[3],"sub-message": "" if len(args) <= 4 else " ".join(args[4:]) }, "end")
        elif len(args) == 4:
...
async def c_settitle (cmd: ChatCommand):
    if (await isStreamer(cmd.user.name)) or await isModerator(cmd.user.name) and len(cmd.parameter) > 0:
...
## Add Quote =======================================================================
async def c_addquote (cmd: ChatCommand):
    pass
## =================================================================================
        ...
        if t - sv["sfx"]["global-usage"][user] >= sv["sfx"]["global-cooldown"]:
            if t - data["last-use-time"][user] >= data["global-cooldown"]:
                ...
                if t - data["last-use-user"][user] >= data["user-cooldown"]:
                    threading.Thread(target=playsound, args=(os.getcwd() + "\\data\\resources\\sounds\\" + data["sound"],), daemon=True).start()
...
async def isModerator (id, user):
    async for mod in sv["twitch"].get_moderators(sv[id]):
...
async def runActions (actions, variables):
    ...
    try:
        ...
        for a in actions:
            ...
            for key, value in adata.items():
                ...
                if "%rlist" in adata[key]:
                    name = adata[key].replace("%","").split(":")[1].split("%")[0]
            ...
            if action == "obs:slideshow":
                cl.trigger_media_input_action(adata["source"], "OBS_WEBSOCKET_MEDIA_INPUT_ACTION_PLAY" if "play" == adata["state"] else "OBS_WEBSOCKET_MEDIA_INPUT_ACTION_PAUSE")
            ## =========================================================================
            ...
            if action == "conditional":
                with open(os.getcwd() + "\\configuration\\conditional-actions.yml", 'r', encoding = "utf-8") as file:
                    ...
                    if adata["name"] in data["conditionals"]:
                        ...
                        datavalue = ""
                        options = { "Counter":"counter", "Variable":"text", "List":"list", "Boolean":"bool"}
                        
                        for option, tag in options.items():
                            if option.lower() in condition.lower():
                                with open(os.getcwd() + "\\data\\variables\\" + tag + "\\" + variable + ".txt", 'r', encoding = "utf-8") as file:
                                    datavalue = file.read()
                        
                        result = None
                        if "counter" in condition.lower():
                            value = int(value)
                            datavalue = int(datavalue)
                            c = {"less than value":(datavalue < value), "less than or equals value":(datavalue <= value), "equals value":(datavalue == value), "more than or equals value":(datavalue >= value), "more than value":(datavalue > value)}
                            for k,v in c.items():
                                if k in condition.lower():
                                    result = v
                                    
                        if "text" in condition.lower():
                            value = str(value)
                            c = {"equals":(datavalue == value), "contains":(value in datavalue)}
                            for k,v in c.items():
                                if k in condition.lower():
                                    result = v
                                    
                        if "list" in condition.lower():
                            value = str(value)
                            c = {"contains":(value in datavalue.splitlines())}
                            for k,v in c.items():
                                if k in condition.lower():
                                    result = v
                                    
                        if "boolean" in condition.lower():
                            result = datavalue == "True"
                            
                        if result is not None:
                            await runActions(data["conditionals"][adata["name"]][result], variables)
            ## =========================================================================
                    ...
                    for voicepack in tts_voice.values():
                        if adata["voice"] in voicepack:
                            ...
                            if voicepack == "acapela":
                                ...
                                params = { 'cl_login': json_res['login'], 'cl_app': json_res['app'],   'session_start': json_res['session']['start'], 'session_time': json_res['session']['time'], 'session_key': json_res['session']['key'], 'req_voice': voice, 'req_text': text }
                            ...
                            if voicepack == "cepstral":
                                ...
                                params = { 'voiceText': text, 'voice': voice, 'createTime': int(time.time() * 1000), 'rate': 170, 'pitch': 1, 'sfx': 'none' }
                            ...
                            if voicepack == "oddcast":
                                ...
                                params = { 'EID': int(engineId), 'LID': int(languageId), 'VID': int(voiceId), 'TXT':text, 'EXT':'mp3', 'FNAME':'', 'ACC':15679, 'SceneID':2703396, 'HTTP_ERR':'', 'cache_flag':3}
                                headers = {
                            ...
                            if voicepack == "voiceforge":
                                ...
                                params = { 'voice':voice, 'msg':text, 'email':'null', }
                            ...
                            if data is not None:
                                ...
                                while True:
                                    ...
                                    await asyncio.sleep(0.1)  # Shorter delay for quicker checks
        ...
        await logError(["Running Action: " + reference])
...
async def logError (info = None):
    ...
    with open (os.getcwd() + "\\data\\logs\\" + timestamp + ".txt", 'w') as log_file:
        log_file.write("User: " + sv["channel"] + "\n" "Version: " + sv["version"] + "\n")
        if info:
            log_file.write("Info:" + "\n")  
            for line in info:
                log_file.write("- " + line + "\n") 
