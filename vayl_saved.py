...
os.system("title Vayl Dashboard")
...
def resize_console(width=60, height=10):
    # Resize the *buffer* (must be >= window size)
    os.system(f"mode con: cols={width} lines={height}")

    # Optional: force window size to match buffer size exactly
    hwnd = ctypes.windll.kernel32.GetConsoleWindow()
    if hwnd:
        # Adjust window rectangle
        # 0 = SW_HIDE, 3 = SW_MAXIMIZE, 9 = SW_RESTORE, 5 = SW_SHOW
...
# ---------------------------------------------------------------------
# Flask listener for spinner results
# ---------------------------------------------------------------------
flask_app = Flask("VaylSpinner")
CORS(flask_app)

pending_spins = []

@flask_app.get("/pollspin")
def poll_spin():
    ...
    # Check if the spinner name is in the queue
    if name in pending_spins:
        pending_spins.remove(name)
        return {"cmd": "spin"}
...
@flask_app.post("/spinresult")
def receive_spin_result():
    ...
    try:
        ...
        asyncio.run(addAlert({"type":"vayl-spinner", "result":result}, "0"))
    
        with open(os.path.join(vdir["vayl"], "latest-spinner-result.txt"), "w", encoding="utf-8") as f:
            f.write(result or "")
...
async def on_ready (ready_event: EventData):
    try:
        ...
        prompt ("success", "Loaded", "Welcome to Vayl")
        await reload(False)
    except Exception as e:
        ...
        if "!addquote" in msg.text:
            try:
                if msg.reply_parent_msg_body:
                    ...
                    quote_message = " ".join(msg.reply_parent_msg_body.split())
...
async def on_redeem_update (data: ChannelPointsCustomRewardRedemptionUpdateEvent):
    try:
        ...
        alert["type"] = "redemption-update"
        await addAlert(alert, "0")
        
    except:
        logError(tag = "event.on_redeem_update")
...
async def c_custom (cmd: ChatCommand):
    try:
        ...
        for cmds in sv["commands"].keys():
            if cmds.lower() == cmd.name.lower() or cmd.name.lower() in sv["commands"][cmds]["alias"]:
    ...
    for a in actions:
        ...
        if action == "counter2":
                try:
                    ...
                    counter = safe_eval_counter(adata["equation"])
                    try:
        ...
        if action == "console":
            try:
                prompt ("misc", "Vayl", adata["message"])
            except Exception as e:
        ...
        if action == "spinner":
            pending_spins.append(adata["name"])
    ...
    try:
        with open(os.getcwd() + "\\configuration\\commands.yml", 'r', encoding = "utf-8") as file:
            ...
            for command in data["command"].keys():
                ...
                aliases = data["command"][command].get("alias", [])
                for alias in aliases:
                    sv["chat"].register_command(alias.lower(), c_custom)
...
def logError(tag = None, additional_details = None):
    ...
    prompt("error", "Error Detected", "Cause: " + reference)
...
async def run():
    ...
    prompt ("success", "Launching Vayl", "Initialization")
    prompt ("success", "Launching Vayl", "Authentication")
    ...
    prompt ("success", "Launching Vayl", "Fetching Twitch User")
    
    sv["streamer"] = await first(sv["twitch"].get_users(logins = [sv["channel"]]))
    
    prompt ("success", "Launching Vayl", "Registering EventSub")
    ...
    prompt ("success", "Launching Vayl", "Connecting to Chat")
...
async def console_ui():
    with Live(console=console, refresh_per_second=8, screen=False) as live:
        while True:
            ...
            live.update(Panel(log_panel.renderable, title="", border_style="white"))

            await asyncio.sleep(0.25)

# --- Prompt function ---
def prompt(category: str, header: str, message: str):
    ...
    if vayl_websocket:
        ...
        asyncio.create_task(
            vayl_websocket.send(json.dumps(payload))
        )
    else:
        pass
...
def safe_path(relative_path: str) -> pathlib.Path:
    ...
    if ROOT not in p.parents:
        raise ValueError("Invalid file path (security)")
    return p
...
def scan_folder(folder: str):
    ...
    return [
        ...
        if path.is_file() and path.suffix in [".yml", ".txt"]
    ]


def scan_variable_folders(base_folder: str):
    ...
    for folder in base.iterdir():
        if folder.is_dir():
            ...
            for file in folder.iterdir():
                if file.is_file() and file.suffix in [".txt", ".yml"]:
                    files.append({
                        "display": file.stem.replace("-", " ").title(),
                        "path": f"{base_folder}/{folder.name}/{file.name}"
                    })

            if files:
                groups.append({
                    "type": folder.name.title(),
                    "files": files
                })
    ...
    def walk(path: pathlib.Path):
        ...
        for entry in sorted(path.iterdir(), key=lambda p: (p.is_file(), p.name.lower())):
            ...
            if entry.is_dir():
                ...
                if entry.suffix.lower() in [".yml", ".yaml", ".txt", ".json"]:
                    items.append({
                        "type": "file",
                        "name": entry.name,
                        "path": rel_path
...
async def send_file(websocket, path):
    try:
        ...
        await websocket.send(f"[ERROR] Could not open file {path}: {e}")


async def save_file(path, content, websocket):
    try:
        real = safe_path(path)
        ...
        # Monaco = \n
        # Windows editors expect \r\n
        content = content.replace("\r\n", "\n")  # collapse any CRLF Monaco accidentally introduces
        content = content.replace("\r", "\n")    # remove stray CR
        content = content.replace("\n", "\r\n")  # final normalisation

        with open(real, "w", encoding="utf8", newline="") as f:
            f.write(content)

        await websocket.send("Saved.")

    except Exception as e:
        await websocket.send(f"[ERROR] Failed to save file {path}: {e}")




async def create_item(websocket, data):
    try:
        path = data["path"]
...
def get_actionpack_list():
    base = os.path.join(os.getcwd(), "configuration", "actionpacks")
    result = []

    if not os.path.exists(base):
        # print("ACTIONPACK PATH NOT FOUND:", base)
        return result
    ...
    for root, dirs, files in os.walk(base):
        for file in files:
            ...
            # path relative to actionpacks/ (used by JS)
            rel_path = os.path.relpath(full_path, base).replace("\\", "/")

            # folder relative to actionpacks/ ("" if file directly in actionpacks/)
            rel_folder = os.path.dirname(rel_path).replace("\\", "/")

            result.append({
                "folder": rel_folder,   # "" or "sub", or "sub/nested"
                "file": file,
                "path": rel_path        # e.g. "test.yml" or "sub/test.yml" or "sub/nested/test.yml"
            })

    # print("ACTIONPACKS FOUND:", result)
    return result
...
async def handler(websocket):
    ...
    global vayl_websocket
    vayl_websocket = websocket
    ws_ready.set()
    ...
    # asyncio.create_task(stream_logs(websocket))
    ...
    try:
        async for msg in websocket:
            ...
            if action == "read":
                await send_file(websocket, data["path"])

            elif action == "write":
                await save_file(data["path"], data["content"], websocket)
            elif action == "create":
                await create_item(websocket, data)


            elif action == "getMacro":
                
                path = os.path.join(os.getcwd(), "dashboard", "macros.json")
                ...
                with open(path, "r", encoding="utf-8") as f:
                    ...
                    await websocket.send(json.dumps({
                        "type": "macro-data",
                        "macro": json.load(f),
                        "actionpacks": get_actionpack_list()
                    }))

            elif action == "setMacro":
                path = os.path.join(os.getcwd(), "dashboard", "macros.json")
                with open(path, "w", encoding="utf-8") as f:
                    json.dump(data["data"], f, indent=2)

            elif action == "runMacro":
                
                try:
                    actionpack = data.get("actionpack", "")
                    
                    full_path = os.path.join(os.getcwd(), "configuration", "actionpacks", actionpack)
